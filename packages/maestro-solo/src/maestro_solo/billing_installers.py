...
def _installer_fleet_windows_script_url() -> str:
    configured = _first_env_value("MAESTRO_INSTALLER_FLEET_WINDOWS_SCRIPT_URL")
    if configured:
        return configured
    return f"{_installer_script_base_url().rstrip('/')}/install-maestro-fleet-windows.ps1"
...
def _build_bash_launcher_script(
    *,
    comment: str,
    ...
    _append_download_and_exec(lines, script_url, tmp_prefix=tmp_prefix)
    return "\n".join(lines) + "\n"
...
def build_installer_script(*, flow: str, billing_base_url: str, intent: str = "") -> str:
    ...
    return _build_bash_launcher_script(
        comment="# Generated by Maestro installer launcher.",
        env_assignments=env_assignments,
        script_url=script_url,
        tmp_prefix="maestro-installer-launcher",
    )
...
def build_fleet_installer_script(*, billing_base_url: str) -> str:
    ...
    return _build_bash_launcher_script(
        comment="# Generated by Maestro Fleet installer launcher.",
        env_assignments=env_assignments,
        script_url=_installer_fleet_script_url(),
        tmp_prefix="maestro-fleet-launcher",
    )


def build_fleet_windows_installer_script(*, billing_base_url: str) -> str:
    fleet_spec = _installer_fleet_package_spec()
    if not fleet_spec:
        raise HTTPException(status_code=503, detail="installer_not_configured:missing_fleet_package_spec")
    ...
    script_url = _installer_fleet_windows_script_url()
    lines = [
        "# Generated by Maestro Fleet Windows installer launcher.",
        "$ErrorActionPreference = 'Stop'",
        "$ProgressPreference = 'SilentlyContinue'",
    ]
    for key, value in env_assignments:
        lines.append(f"$env:{key} = {_powershell_single_quote(value)}")
    lines.extend(
        [
            "$tmpScript = Join-Path ([System.IO.Path]::GetTempPath()) ('maestro-fleet-launcher-' + [guid]::NewGuid().ToString() + '.ps1')",
            "try {",
            f"  Invoke-WebRequest -UseBasicParsing -Uri {_powershell_single_quote(script_url)} -OutFile $tmpScript",
            "  & powershell -NoProfile -ExecutionPolicy Bypass -File $tmpScript @args",
